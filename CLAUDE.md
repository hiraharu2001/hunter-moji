...
**ルーティング**: hash ルート（`#/to-hunter` `#/to-kana` `#/chart`）を `useHashRoute`（`useSyncExternalStore` で `hashchange` を購読）で扱う。GitHub Pages のリロード 404 を避けるため BrowserRouter は使わない。
...
**テストの重心**: 変換ロジック（`src/lib/*.test.ts`）と字形の不変条件（`src/glyphs/glyphs.test.ts` の「対応表の全文字に字形がある」「字形が互いに異なる」「座標が viewBox 内」）。UI（`src/components/`）にはテストが無い。
